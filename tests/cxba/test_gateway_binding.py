...
def test_trusted_cxba_agent_skips_repository_context_but_keeps_soul(monkeypatch):
    ...
    try:
        ...
        assert "cxba-analysis-pitfalls" in kwargs["ephemeral_system_prompt"]
        assert "cxba-claim-delivery" in kwargs["ephemeral_system_prompt"]
        assert "main files successfully created or modified" in kwargs["ephemeral_system_prompt"]
        assert "Markdown inline code" in kwargs["ephemeral_system_prompt"]
