        ...
        "claims": [
            {
                ...
                "calculationRefs": [
                    {"scriptPath": "scripts/calc.py", "resultPath": "results/calc.json"}
                ],
            }
        ]
    }
...
def test_claim_delivery_checks_structured_material_id_in_result(tmp_path: Path) -> None:
    ...
    claim = {
        ...
        "calculationRefs": [{"scriptPath": "scripts/calc.py", "resultPath": "results/calc.json"}],
    }
    (workspace / "evidence-items" / "final-claims.json").write_text(
...
def _write_minimal_xlsx(path: Path) -> None:
    with zipfile.ZipFile(path, "w") as archive:
        ...
        archive.writestr("xl/sharedStrings.xml", "<sst><si><t>唐兆烨</t></si></sst>")
...
def test_claim_delivery_validates_xlsx_and_publication_reconciliation(tmp_path: Path) -> None:
    ...
    (workspace / "input" / "materials.json").write_text(
        json.dumps(
            [{"materialId": "material-cn", "relativePath": "材料/唐兆烨.xlsx"}],
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
    (workspace / "scripts" / "汇总.py").write_text("# fixture\n", encoding="utf-8")
    workbook = workspace / "results" / "唐兆烨_交易对手汇总.xlsx"
        ...
        "sourceRefs": [
            {
                ...
                "relativePath": "材料/唐兆烨.xlsx",
                "role": "SUPPORT",
                "locatorType": "EXCEL_RANGE",
                "locator": {"sheet": "唐兆烨", "startRow": 2, "endRow": 903},
            }
        ],
        "calculationRefs": [
            {
                ...
                "artifactPaths": ["results/唐兆烨_交易对手汇总.xlsx"],
            }
        ],
    }
...
def test_round_three_rules_are_explicit() -> None:
    ...
    assert "下一次内容工具" in contents
    assert "批量伪回填" in contents
    for field in (
        "文件",
        "Sheet/页",
        "Excel原始行/唯一流水",
