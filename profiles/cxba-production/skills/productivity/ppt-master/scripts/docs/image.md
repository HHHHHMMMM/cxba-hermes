...
Use provider-specific keys only (e.g. `GEMINI_API_KEY`, `OPENAI_API_KEY`). See `.env.example` in clone mode or `/root/.hermes/skills/productivity/ppt-master/.env.example` in skill-install mode for the full list per backend.
