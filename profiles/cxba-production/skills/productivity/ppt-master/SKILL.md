...
name: ppt-master
description: >
  Offline PPT production from authorized local PDF, DOCX, XLSX, PPTX, Markdown,
  text and images. Uses bundled layouts, charts and icons to generate SVG pages
  and export PPTX. Use for 创建PPT、制作演示文稿、汇报材料 or ppt-master.
---
...
> [!IMPORTANT]
> ## CXBA Hermes Offline Runtime Overrides
>
> These rules override any conflicting instruction later in this Skill:
>
> 1. The read-only runtime directory is `/root/.hermes/skills/productivity/ppt-master`; always use absolute paths beneath it for scripts, templates and references.
> 2. Run scripts with the Sandbox `python`. Never use `.venv`, `pip`, `npm`, `npx`, `apt` or any runtime installer.
> 3. Read source material only from the current authorized `/data`, `/workspace` and user-provided attachments. Original material is immutable: never move, rename or modify it. Import with `--copy`, and write projects and all generated artifacts only below `/workspace`.
> 4. The Sandbox has no network. Do not run topic research, URL/WeChat conversion, web image search, cloud image generation, TTS or any other network workflow.
> 5. Use bundled layouts, charts, icons, user-provided images or explicit placeholders. Every image resource must use `Acquire Via: user` or `Acquire Via: placeholder`; skip the online Image Acquisition phase.
> 6. Browser live preview and annotation are unavailable inside the Sandbox. Skip preview startup and annotation handling; use `svg_quality_checker.py`, local rendered files and the final PPTX export as the verification path.
...
| Script | Purpose |
|--------|---------|
| `/root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/pdf_to_md.py` | PDF to Markdown |
| `/root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/doc_to_md.py` | Documents to Markdown — native Python for DOCX/HTML/EPUB/IPYNB, pandoc fallback for legacy formats (.doc/.odt/.rtf/.tex/.rst/.org/.typ) |
| `/root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/excel_to_md.py` | Excel workbooks to Markdown — supports .xlsx/.xlsm; legacy .xls should be resaved as .xlsx |
| `/root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/ppt_to_md.py` | PowerPoint to Markdown |
| `/root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/web_to_md.py` | Web page to Markdown (supports WeChat via `curl_cffi`) |
| `/root/.hermes/skills/productivity/ppt-master/scripts/project_manager.py` | Project init / validate / manage |
| `/root/.hermes/skills/productivity/ppt-master/scripts/analyze_images.py` | Image analysis |
| `/root/.hermes/skills/productivity/ppt-master/scripts/image_gen.py` | AI image generation (multi-provider) |
| `/root/.hermes/skills/productivity/ppt-master/scripts/svg_quality_checker.py` | SVG quality check |
| `/root/.hermes/skills/productivity/ppt-master/scripts/total_md_split.py` | Speaker notes splitting |
| `/root/.hermes/skills/productivity/ppt-master/scripts/finalize_svg.py` | SVG post-processing (unified entry) |
| `/root/.hermes/skills/productivity/ppt-master/scripts/svg_to_pptx.py` | Export to PPTX |
| `/root/.hermes/skills/productivity/ppt-master/scripts/update_spec.py` | Propagate a `spec_lock.md` color / font_family change across all generated SVGs |

For complete tool documentation, see `/root/.hermes/skills/productivity/ppt-master/scripts/README.md`.

## Template Index

| Index | Path | Purpose |
|-------|------|---------|
| Layout templates | `/root/.hermes/skills/productivity/ppt-master/templates/layouts/layouts_index.json` | Query available page layout templates |
| Visualization templates | `/root/.hermes/skills/productivity/ppt-master/templates/charts/charts_index.json` | Query available visualization SVG templates (charts, infographics, diagrams, frameworks) |
| Icon library | `/root/.hermes/skills/productivity/ppt-master/templates/icons/` | See `/root/.hermes/skills/productivity/ppt-master/templates/icons/README.md`; search icons on demand with `ls templates/icons/<library>/ \| grep <keyword>` |
...
> **No source content?** Ask the user for source material or a substantive local brief. Do not run `topic-research` in the CXBA offline runtime.

When the user provides non-Markdown content, convert immediately:

| User Provides | Command |
|---------------|---------|
| PDF file | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/pdf_to_md.py <file>` |
| DOCX / Word / Office document | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/doc_to_md.py <file>` |
| XLSX / XLSM / Excel workbook | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/excel_to_md.py <file>` |
| CSV / TSV | Read directly as plain-text table source |
| PPTX / PowerPoint deck | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/ppt_to_md.py <file>` |
| EPUB / HTML / LaTeX / RST / other | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/source_to_md/doc_to_md.py <file>` |
| Web link | Unsupported in the CXBA offline runtime; ask for an authorized downloaded file |
| WeChat / high-security site | Unsupported in the CXBA offline runtime; ask for an authorized downloaded file |
...
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/project_manager.py init <project_name> --format <format>
```
...
| Situation | Action |
|-----------|--------|
| Has source files (PDF/MD/etc.) | `python /root/.hermes/skills/productivity/ppt-master/scripts/project_manager.py import-sources <project_path> <source_files...> --copy` |
| User provided text directly in conversation | No import needed — content is already in conversation context; subsequent steps can reference it directly |

> ⚠️ **CXBA immutable-source rule:** MUST use `--copy`. Never use `--move`; `/data` and authorized attachments remain unchanged. Imported working copies and intermediate artifacts belong under the `/workspace` project.
...
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/analyze_images.py <project_path>/images
```
...
> **CXBA offline rule:** Do not enter the online acquisition paths below. Use bundled templates/icons, authorized user images or placeholders, then continue directly to Step 6.
...
|---|---|---|
| `ai` | `references/image-generator.md` | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/image_gen.py --manifest <project_path>/images/image_prompts.json` |
| `web` | `references/image-searcher.md` | `python3 /root/.hermes/skills/productivity/ppt-master/scripts/image_search.py ...` |
...
**CXBA offline preview rule:** browser live preview is unavailable in the Run Sandbox. Do not start the editor below; generate SVG pages sequentially and use the quality checker before export.

<!-- Upstream optional preview command retained for future connected profiles:
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/svg_editor/server.py <project_path> --live
```
-->
...
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/svg_quality_checker.py <project_path>
```
...
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/total_md_split.py <project_path>
```

**Step 7.2** — SVG post-processing (icon embedding / image crop & embed / text flattening / rounded rect to path):
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/finalize_svg.py <project_path>
```

**Step 7.3** — Export PPTX (embeds speaker notes by default):
```bash
python3 /root/.hermes/skills/productivity/ppt-master/scripts/svg_to_pptx.py <project_path>
# Output:
