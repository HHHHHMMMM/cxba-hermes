...
编写或修改费用适配器/临时脚本，或者开展金额、方向、去重和结果复核前，加载`cxba-analysis-pitfalls`并按需读取已验证规则；当前上下文已完整读取时不重复加载。
...
当用户要求“整体看看”“全面分析费用”或同时关心费用申请与实际收款，而原件中申请人和收款方是两个不同业务角色时，必须分别以申请人和收款方运行排名、集中度和候选分析，再核对两类角色的交集与差异；不得只选其中一个角色代替整体费用分析。某一角色字段缺失或无法可靠映射时明确记录缺口，不得把另一角色的零命中外推为该角色也无问题。
...
用户未指定阈值而要求整体发现候选时，先完成不设金额阈值的全量主体排名和分布分析，再从金额、频次、集中度、相对基线和时间等客观维度选取有限候选。后续为深入核查设置阈值时，必须说明阈值来源和被阈值排除的范围；阈值内零命中只回答该有界口径，不代表整体不存在其他规律候选。
