...
处理完一个物理文件后，在下一次读取、解析、搜索、计算或委派等内容工具调用前立即回写该记录。禁止处理多文件或结束时批量伪回填。失败保留`FAILED`，部分覆盖保留`PARTIAL`。
...
第一次分析内容、每个物理文件后下一内容工具前、有意义发现后、连续3次工具调用仍未完成文件、接收子Agent结果后、切换主线和写报告前，先更新状态与台账。无发现也记录覆盖、失败、限制和下一步。
