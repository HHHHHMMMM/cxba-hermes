...
不得凭记忆猜JSON结构。计算Claim至少使用以下原样字段名和层级；字段名区分大小写，完整定位和其他字段继续以关联合同为准：

```json
{
  "claimCode": "C001",
  "statement": "由结果JSON的reportBlock原样生成的陈述",
  ...
  "sourceRefs": [
    {
      ...
      "relativePath": "与materialId精确对应的完整相对路径",
      "role": "SUPPORT | COUNTER",
      "locatorType": "EXCEL_RANGE",
      "locator": {"sheet": "真实Sheet名", "rows": [2, 5]},
      "description": "该原始位置具体支持或反驳什么"
    }
  ],
  "calculationRefs": [
    {
      ...
      "artifactPaths": [],
      "purpose": "这段计算要证明什么",
      "calculationBasis": "字段、筛选、方向、期间、币种、状态、空值、冲正及去重口径"
    }
  ]
}
```
...
4. `terminal`会把脚本的完整标准输出和退出码返回当前主Agent。必须完整读取`CLAIM_DELIVERY_FAIL`后的错误和`修正方式`。错误较少时，按Claim序号、引用序号和字段名逐项修正相关Claim、笔记、脚本、结果或回复；错误较多、出现多个必填字段缺失或输出提示`REGENERATE_FROM_SKILL`时，不得继续猜字段或零散打补丁，必须重新通过`skill_view`完整读取`cxba-claim-delivery`及`references/final-claim-delivery.md`，按合同重新生成本轮`final-claims.json`和`final-answer.md`，再运行同一预检命令。不得使用`|| true`、忽略退出码、截掉错误输出，或把“工具调用已结束”误当成校验通过。
5. 工具会同时检查Claim必填字段、最终回复、分析笔记及Claim实际引用的计算脚本和结果中的材料身份；按文件类型校验二进制交付物结构，并检查计算结果、交付物回读值和发布文本勾稽。只要缺少`description`、`purpose`或`calculationBasis`，或者存在路径与`materialId`不一致、二进制格式损坏、复算值不一致或`reportBlock`未原样发布，证据校验即失败。
