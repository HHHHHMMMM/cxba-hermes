...
薄清单中的每个物理文件都必须单独检查并对齐台账；`.DS_Store`保留在覆盖对账中但可标为`NON_MATERIAL`，ZIP必须记录并检查内部成员。每处理完一个物理文件，必须在下一次内容工具调用前立即回写该文件台账，禁止在结束时批量伪回填。发现线索时立即记录精确定位（原文件、Sheet或页码、Excel原始行或唯一流水、内容摘要）、字段角色、收付方向、口径和核验状态；不得只把发现保留在对话上下文或工具输出中。
